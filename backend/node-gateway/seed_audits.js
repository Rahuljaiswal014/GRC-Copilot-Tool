...
async function seed() {
  console.log('Seeding Internal Audit and Vendor Assessment frameworks...');
  
  try {
    const frameworks = [
      { name: 'Internal Audit', desc: 'Comprehensive internal security assessment' },
      { name: 'Vendor Assessment', desc: 'Third-party security and privacy review' }
    ];

    for (const fw of frameworks) {
      const fwResult = await pool.query(
        'INSERT INTO frameworks (name, description) VALUES ($1, $2) ON CONFLICT (name) DO UPDATE SET description = EXCLUDED.description RETURNING id',
        [fw.name, fw.desc]
      );
      const fwId = fwResult.rows[0].id;
      const type = fw.name === 'Internal Audit' ? 'internal' : 'vendor';
      
      for (const q of AUDIT_QUESTIONS[type]) {
        // Create Control
        const controlResult = await pool.query(
          'INSERT INTO controls (framework_id, control_id, name, domain) VALUES ($1, $2, $3, $4) ON CONFLICT (framework_id, control_id) DO UPDATE SET name = EXCLUDED.name RETURNING id',
          [fwId, q.id, q.text.substring(0, 100), q.category]
        );
        const controlId = controlResult.rows[0].id;

        // Create Question
        await pool.query(
          `INSERT INTO questions (framework_id, control_id, question_id, text, hint, response_type, weight)
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           ON CONFLICT (framework_id, question_id) DO UPDATE SET text = EXCLUDED.text`,
          [fwId, controlId, q.id, q.text, q.hint, 'maturity', 1.0]
        );
      }
    }
